...
import { buildPitches, midiToFreq, SCALES, colorForDegree, DEGREE_COLORS } from "./scale";

describe("scale", () => {
  it("spans N octaves of the scale plus the closing tonic", () => {
    const pitches = buildPitches(60, "major", 2);
    // 7 degrees * 2 octaves + the top tonic.
    expect(pitches).toHaveLength(SCALES.major.length * 2 + 1);
    expect(pitches[0].midi).toBe(60);
    expect(pitches[pitches.length - 1].midi).toBe(60 + 24);
  });
